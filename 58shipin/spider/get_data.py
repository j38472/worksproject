...
class ShiPin:
    ...
    def get_response(self, url):
        s = 1
        try:
            response = requests.get(url=url, headers=self.headers, timeout=10, stream=True)
            # print(response.status_code, type(response.status_code))
            print(response.status_code)
            # 网页获取错误，重新获取网页
            if response.status_code != 200:
                print('第%s次获取网页' % s, url)
                s += 1
                sleep(randint(3, 6))
    ...
    def page_urls(self):
        if self.page < self.pages + 1:
            # 拼接分类页码网址
            page_url = 'http://www.58food.com/qy-l-0-0-7072-0-%d.html' % self.page
    ...
    def save_data(self, dic):
        try:
            ...
            sql1 = """
                insert into mimianliangyou (company, produce, company_type, name, phone, telephone, address, href) values (%s, %s, %s, %s, %s, %s, %s, %s);
                           """
...
if __name__ == '__main__':
    url = 'http://www.58food.com/qy-l-0-7072.html'
    T = ShiPin()
    T(url)
