...
class ShiPin:
    ...
    def get_response(self, url):
        ...
        try:
            # response = requests.get(url=url, headers=self.headers, proxies=proxies, timeout=5, stream=True)
            # page_text = requests.get('http://httpbin.org/ip', headers=self.headers, proxies=proxies).text
            # print(page_text)
    ...
    def get_pages(self, t_url):
        ...
        if self.page == 1:
            ...
            sleep(0.3)
            self.get_data(t_url)
            self.get_pages(t_url)
        elif self.page > 1:
            # 需要更改的代码
            for pn in range(2, self.pages + 1):
            # for pn in range(188, 383):
                # print('新一页')
                # 拼接分类页码网址
                page_url = 'http://www.58food.com/qy-l-0-0-3903-0-%d.html' % pn
                print('数据获取中，第%s页' % pn)
                self.page += 1
    ...
    def save_data(self, dic):
        try:
            ...
            sql1 = """
                insert into shipinpeiliao (company, produce, company_type, name, phone, telephone, address, href) values (%s, %s, %s, %s, %s, %s, %s, %s);
                           """
...
if __name__ == '__main__':
    # 数据
    url = 'http://www.58food.com/qy-l-0-3903.html'
    T = ShiPin()
    T(url)
